...
class LandlordListingController extends Controller
{
    ...
    {
        $newFileName = $filename.'-'.$request->user()->first_name.'-'.$request->user()->last_name;
        return $newFileName;
    }
    ...
    public function submit_listing_documents(Request $request)
    {
        ...
        $request->file('gas_certificate')->storeAs('listing/documents', $gasCertificate);
        ...
        $request->file('electrical_certificate')->storeAs('listing/documents', $electricalCertificate);
        ...
        $request->file('detectors_certificate')->storeAs('listing/documents', $detectorsCertificate);
        ...
        $request->file('emergency_lighting_certificate')->storeAs('listing/documents', $emergency_lightingCertificate);
        ...
        $request->file('fire_risk_certificate')->storeAs('listing/documents', $fire_riskCertificate);
        ...
        $request->file('pat_certificate')->storeAs('listing/documents', $patCertificate);
        ...
        $request->file('insurance_certificate')->storeAs('listing/documents', $insuranceCertificate);
        ...
        $request->file('ownership_certificate')->storeAs('listing/documents', $ownershipCertificate);
    ...
    public function submit_listing_images(Request $request)
    {
        ...
        if ($request->file()) {
            $filename = $request->file->getClientOriginalName().'-'.time();
            $filepath = $request->file('file')->storeAs('listing/images', $filename);
    ...
    public function delete_listing($id)
    {
        ...
        foreach ($listingImages as $image) {
            Storage::delete('/storage/listing/images/', $image);
        }

        $listingDocuments = ListingDocuments::where('listing_id', '=', $id);
        foreach ($listingDocuments as $document) {
            Storage::delete('/storage/listing/documents', $document);
        }
